...
import (
	...
	"github.com/sapcc/argora/test/utils"
)
...
var (
	...
	projectImage = "github.com/sapcc/argora:v0.0.1"
)
...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting argora integration test suite\n")
...
var _ = BeforeSuite(func() {
	...
	_, err := utils.Run(cmd)
	ExpectWithOffset(1, err).ToNot(HaveOccurred(), "Failed to build the manager(Operator) image")

	// TODO(user): If you want to change the e2e test vendor from Kind, ensure the image is
	// built and available before running the tests. Also, remove the following block.
	By("loading the manager(Operator) image on Kind")
	err = utils.LoadImageToKindClusterWithName(projectImage)
	ExpectWithOffset(1, err).ToNot(HaveOccurred(), "Failed to load the manager(Operator) image into Kind")
