...
prepare-deploy:
	kustomize build config/default | yq ea 'select(.kind == "ConfigMap")' > dist/chart/templates/configmap.yaml
	kustomize build config/default | yq ea 'select(.kind == "Secret")' > dist/chart/templates/secret.yaml
...
helm-build-local-image: prepare-deploy helm-lint

.PHONY: helm-build
helm-build: set-image prepare-deploy helm-lint
...
build/cover.out: FORCE install-ginkgo generate install-setup-envtest | build
	...
	KUBEBUILDER_ASSETS=$$(setup-envtest use 1.33 -p path) ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=argora -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTPKGS)
